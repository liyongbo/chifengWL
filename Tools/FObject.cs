...
namespace Tools
{
    ...
    public abstract class FObject
    {
        ...
        public static void ZipFilesList(List<string> lstFilesPath, string strZip, string IISPath)
        {
            string sTemPath = HttpContext.Current.Server.MapPath(string.Concat("UploadFile/temp/", Path.GetRandomFileName(), "/"));
            if (!FObject.IsExist(sTemPath, FsoMethod.Folder))
            {
                Create(sTemPath, FsoMethod.Folder);
                string sFileName = "";
                foreach (string sPath in lstFilesPath)
                {
                    sFileName =GetString.GetFileName(sPath);
