...
namespace Tools
{
    public class GetString
    {
        ...
        /// 
        /// </summary>
        /// <param name="vis">标识（必须成对如：{}、[]的英文符号）</param>
        /// <param name="content">内容</param>
        /// <returns></returns>
        public static string[] GetRegex(string vid, string content)
        {
            char[] st = vid.ToCharArray();
            return Regex.Matches(content, string.Concat(@"(?<=", st[0], @")\w+?(?=", st[1], ")")).OfType<Match>().Select(match => match.Value).ToArray();
        }
        /// <summary>
        /// 
        /// </summary>
        ...
        /// <returns></returns>
        public static string[] GetStringTO(string startstr, string endstr, string content)
        {
            List<string> list = new List<string>();
            while (content.IndexOf(startstr) > -1)
            {
                list.Add(content.Substring(content.IndexOf(startstr) + 1, (content.IndexOf(endstr) - content.IndexOf(startstr)) - 1));
                content = content.Substring(content.IndexOf(endstr) + 1);
            }
            return list.ToArray();

        }

        //public static string[] GetRegex2(string vid, string content)
        //{
        //    char[] st = vid.ToCharArray();
        //    return Regex.Matches(content, string.Concat("[^[]\w+[]]+")).OfType<Match>().Select(match => match.Value).ToArray();
        //}
        ...
        /// 返回字符数组
        /// </summary>
        /// <param name="files">以“,”组合的字符串</param>
        /// <param name="isEmpty">是否去除空组合</param>
        /// <returns></returns>
        public static string[] getStringArray(string files, bool isEmpty)
        {
            if (isEmpty)
        ...
        public static string GetNewNameByDate(string OldFileName, out string SmallFileName, out string MiddleFileName, out string MiddleFileName2, out string MiddleFileName3, out string BigFileName, out string RuleFileName)
        {
            string str = getFileType(OldFileName);
            string randomFileName = string.Concat(DateTime.Now.ToString("yyyyMMdd"), "/", Path.GetRandomFileName().Replace(".", ""));//某些情况下带点路径不兼用

            SmallFileName = string.Concat(randomFileName, "-small", str);
            MiddleFileName = string.Concat(randomFileName, "-middle", str);
            MiddleFileName2 = string.Concat(randomFileName, "-middle2", str);
            MiddleFileName3 = string.Concat(randomFileName, "-middle3", str);
            BigFileName = string.Concat(randomFileName, "-big", str);
            RuleFileName = string.Concat(randomFileName, "-small{0}", str);
            return string.Concat(randomFileName, str);
        }
        ...
        {
            str = GetString.RegexReplace(str, @"\<.*\>|\[.*\]", "");
            str = GetString.RegexReplace(str, @"\[[a-z][^\]]*\]|\[\/[a-z]+\]", "");
            str = str.Replace("/</", "");
            str = str.Replace("/>/", "");
            str = str.Replace("/\r@\n/", "");
            str = str.Replace("<br>", "");
        ...
        /// <param name="strPhone"></param>
        /// <returns></returns>
        public static string GetStrAfter(string fuhao, string str)
        {
            if (!string.IsNullOrEmpty(str) && !string.IsNullOrEmpty(fuhao))
            {
                int fuindex = str.IndexOf(fuhao);
                if (fuindex > 0)
                {
                    return str.Substring(fuindex);
                }
            }
            return string.Empty;
        }
        ...
        /// <param name="strPhone"></param>
        /// <returns></returns>
        public static string GetStrSmall(string imgurl)
        {
            if (!string.IsNullOrEmpty(imgurl))
            {
                string ext = GetStrAfter(".", imgurl);
                imgurl = imgurl.ToLower().Replace(".jpg", "");
                imgurl = imgurl.ToLower().Replace(".png", "");
                imgurl = imgurl.ToLower().Replace(".jpge", "");
                imgurl = imgurl.ToLower().Replace(".gif", "");
                return imgurl + "-small" + ext;
            }
            return string.Empty;
        }
        ...
        /// <param name="strPhone"></param>
        /// <returns></returns>
        public static string GetStrNormal(string imgurl)
        {
            if (!string.IsNullOrEmpty(imgurl))
            {
                string ext = GetStrAfter(".", imgurl);
        ...
        /// <param name="strPhone"></param>
        /// <returns></returns>
        public static string GetStrBigNormal(string imgurl)
        {
            if (!string.IsNullOrEmpty(imgurl))
            {
                string ext = GetStrAfter(".", imgurl);
                ...
                imgurl = imgurl.ToLower().Replace(".jpge", "");
                imgurl = imgurl.ToLower().Replace(".gif", "");
                return imgurl + "-bigthannormal" + ext;
            }
            return string.Empty;
        }
    }
