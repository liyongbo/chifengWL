...
namespace Data.DataProfile
{
    ...
    public abstract class DbHelperBase
    {
        ...
        /// 数据库连接字符串
        /// </summary>
        public abstract string ConnectionString(string sqlname, string dbName);
        ...
        /// <param name="commandParameters">DbParameter参数数组</param>
        /// <returns>返回命令影响的行数</returns>
        public int ExecuteNonQuery(CommandType commandType, string commandText, string sqlName, string dbName, params DbParameter[] commandParameters)
        {

            if (string.IsNullOrEmpty(ConnectionString(sqlName,dbName))) throw new ArgumentNullException("ConnectionString()");
            using (DbConnection connection = Factory.CreateConnection())
            {
                connection.ConnectionString = ConnectionString(sqlName, dbName);
                connection.Open();
                return ExecuteNonQuery(connection, commandType, commandText, commandParameters);
            }
        }
        ...
        private DbDataReader ExecuteReader(DbConnection connection, DbTransaction transaction, CommandType commandType, string commandText, DbParameter[] commandParameters, DbConnectionOwnership connectionOwnership, out long paramer)
        {
            paramer = 0;
            if (connection == null) throw new ArgumentNullException("connection");
            ...
            try
            {
                ...
                //        canClear = false;
                //}
                //paramer = long.Parse(cmd.Parameters["@doCount"].Value.ToString());
        ...
        /// <returns>返回包含结果集的DbDataReader</returns>
        public DbDataReader ExecuteReader(CommandType commandType, string commandText, out long returnparmer, params DbParameter[] commandParameters)
        {
            if (string.IsNullOrEmpty(ConnectionString())) throw new ArgumentNullException("ConnectionString()");
            ...
            try
            {
                ...
                return ExecuteReader(connection, null, commandType, commandText, commandParameters, DbConnectionOwnership.Internal, out returnparmer);
            }
            catch
            {
        ...
        public DbDataReader ExecuteReader(CommandType commandType, string commandText, string sqlName, string dbName, params DbParameter[] commandParameters)
        {
            if (string.IsNullOrEmpty(ConnectionString(sqlName, dbName))) throw new ArgumentNullException("ConnectionString()");
            DbConnection connection = null;
            try
        ...
        /// <param name="commandParameters">分配给命令的SqlParamter参数数组</param>
        /// <returns>返回结果集中的第一行第一列</returns>
        public object ExecuteScalar(CommandType commandType, string commandText,string sqlName,string dbName, params DbParameter[] commandParameters)
        {
            if (string.IsNullOrEmpty(ConnectionString(sqlName,dbName))) throw new ArgumentNullException("ConnectionString()");
            // 创建并打开数据库连接对象,操作完成释放对象.
            using (DbConnection connection = Factory.CreateConnection())
        ...
        public DbParameter MakeInParam(string ParamName, SqlDbType DbType, int Size, object Value)
        {
            return MakeParam(ParamName, DbType, Size, ParameterDirection.Input, Value);
        }

        public DbParameter MakeOutParam(string ParamName, SqlDbType DbType, int Size)
        {
            return MakeParam(ParamName, DbType, Size, ParameterDirection.Output, null);
        }

        public DbParameter MakeParam(string ParamName, SqlDbType DbType, Int32 Size, ParameterDirection Direction, object Value)
        {
            DbParameter param;
